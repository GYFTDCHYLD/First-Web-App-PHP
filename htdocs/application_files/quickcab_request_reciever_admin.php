           ...
           )
{
 ...
 setcookie (session_name(QUICK_CAB_ADMIN), '', time()-3600, '/'); //delete the session
...
<html lang="en">
	<head>
		...
		<link type="text/css" rel="stylesheet" href="css/gyftdchyld2.css" />
				...
				<script type="text/javascript" src="scripts/js/auto_request_admin.js"></script>
	</head>
	<body>
		
	<center>
                            <!-- Messages -->
		<font size = 3 ><h2><fieldset><legend>QUICK-CAB-ADMIN</legend> 
		<a href='shedule_request_reciever_admin'><font color ="green" size=1>shedule</font></a>
...
if (!$problem)							 
       ...
       //Define the query:
	   	   ...
	   	   $query = "DELETE  FROM.`quick_cab` WHERE `quick_cab`.`msg_id` = $request_id";
	...
	//		 		 $url = BASE_URL . 'application_files/quickcab_request_reciever_admin.php'; // Define the URL.
	//ob_end_clean(); // Delete the buffer.
//	header("Location: $url");
		    }
...
?>		
					      ...
					      <!-- form-->	
		<form action="quickcab_request_reciever_admin.php" method="post">
		
		
	<p><br /><center>REQUEST ID  <input type="number" name="request_id" size="5" required />  <input type="submit" name="submit" value="REMOVE" /></center></p>
	
	<p> ADMINISTRATIVE USER:  <?php if (isset($_SESSION['admin'])) { echo "<font color ='BLUE'> ({$_SESSION['admin']})</font> ";}else{echo "<font color ='red'>(not approved)</font>";}?> </p>
	
	<font color ="gold"><i> property of anblascar</i></p>
	<!--end of form-->
